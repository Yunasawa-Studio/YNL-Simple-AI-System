...
namespace YNL.SimpleAISystem.Editors
{
    ...
    {
        private const string _styleSheet = "Style Sheets/Elements/EStateBox";
        ...
        public EStateBox(EStatePanel panel, AIStateKey state) : base()
        {
            ...
            Texture2D icon = $"Textures/Behaviours/States/{state.Name}".LoadResource<Texture2D>();
        ...
        private void EndRenameState(FocusOutEvent evt)
        {
            ...
            Texture2D icon = $"Textures/Behaviours/States/{Input.value}".LoadResource<Texture2D>();
            if (icon.IsNull()) icon = "Textures/Behaviours/Null".LoadResource<Texture2D>();
